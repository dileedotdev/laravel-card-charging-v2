...
it('should work', function (Status $status): void {
    CardChargingV2ServerFaker::make()->fakeCheck(status: $status);

    $card = CardChargingV2::check('VIETTEL', 10000, '100037483843245', '100037483843245', '33');

    expect($card->status)->toBe($status);
})->with(Status::cases());

it('should work with SUCCESS status', function (): void {
    CardChargingV2ServerFaker::make()->fakeCheck(status: Status::SUCCESS, fees: 28);
...
it('should work with INCORRECT_VALUE_SUCCESS status', function (): void {
    CardChargingV2ServerFaker::make()->fakeCheck(status: Status::INCORRECT_VALUE_SUCCESS, fees: 28, penalty: 50);
...
it('should work with PENDING status', function (): void {
    CardChargingV2ServerFaker::make()->fakeCheck(status: Status::PENDING, fees: 28);
...
it('should work with INCORRECT_CARD status', function (): void {
    CardChargingV2ServerFaker::make()->fakeCheck(status: Status::INCORRECT_CARD, fees: 28);
